 ...
 */
public class MetrixMetricsPoller {

	static final Logger logger = LoggerFactory.getLogger(MetrixMetricsPoller.class);
	 ...
	 * @param delay
	 */
	public MetrixMetricsPoller(MetricsAsJsonPollerListener listener, int delay) {
	...
	public synchronized void start() {
		...
		if (running.compareAndSet(false, true)) {
			logger.info("Starting MetrixMetricsPoller");
			try {
	...
	public synchronized void pause() {
		...
		if (running.compareAndSet(true, false)) {
			logger.info("Stopping the MetrixMetricsPoller");
	...
	private final Object finalizerGuardian = new Object() {
		protected void finalize() throws Throwable {
			if (!executor.isShutdown()) {
				logger.warn(MetrixMetricsPoller.class.getSimpleName() + " was not shutdown. Caught in Finalize Guardian and shutting down.");
				try {
					shutdown();
				} catch (Exception e) {
					logger.error("Failed to shutdown " + MetrixMetricsPoller.class.getSimpleName(), e);
				}
			}
		};
	};
	...
	private class MetricsPoller implements Runnable {
		...
		public void run() {
			try {
				// command metrics
				for (HystrixCommandMetrics commandMetrics : MetrixCommandMetrics.getInstances()) {
